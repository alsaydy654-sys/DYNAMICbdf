...
## 3. Storage
- Creates storage bucket `textbooks` (public-read so uploaded images are viewable)
...
- Storage bucket is public; storage object policies allow anon + authenticated
  to read, insert, update, delete objects in the `textbooks` bucket.

## 5. Notes
- Column names match the DEFAULT_CONFIG.columns mapping in the app. The user can
  remap these at runtime via the Settings panel; if they target a different
  table they must create that table themselves with matching columns.
...
-- 5. Storage object policies (anon + authenticated)
